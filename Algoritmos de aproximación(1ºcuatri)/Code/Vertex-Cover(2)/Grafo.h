...
public:
   ...
   void quitaArista(int v, int w) {
	   if (v >= _V || w >= _V)
		   throw std::invalid_argument("Vertice inexistente");
	   int i = 0;
	   bool findJ = false, findI = false;
	   while (ady(v)[i] != w && i < ady(v).size()) // Buscamos w en ady(v)
		   i++;
	   if (i < ady(v).size()) {
		   _E--;
		   _ady[v].erase(_ady[v].begin() + i);
		   i = 0;
		   while (ady(w)[i] != v && i < ady(w).size()) // Buscamos v en ady(w)
			   i++;
		   _ady[w].erase(_ady[w].begin() + i);
	   }
   }
